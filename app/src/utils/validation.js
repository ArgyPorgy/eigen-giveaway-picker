...
export function validateEthAmount(amount) {
...
export function calculateMinShares(shares, slippagePercent = 1) {
    const slippageBps = BigInt(Math.floor(slippagePercent * 100));
    const slippageAmount = (shares * slippageBps) / 10000n;
    return shares - slippageAmount;
}
export function calculateMinEth(eth, slippagePercent = 1) {
    const slippageBps = BigInt(Math.floor(slippagePercent * 100));
